...
export class LLM {
    ...
    async load(model, options, flag = true) {
        ...
        this.num_layers = 32;
        log(`Loading... ${model.name}, ${this.dtype}, ${this.provider}`);
        ...
        const external_data_bytes = await fetchAndCache(external_data_path);
        ...
        console.log('create session 1 with option: ', { ...session_options });
        this.sess_1 = await ort.InferenceSession.create(model_bytes, session_options);
        log('Create session for prefill... done');
        if (this.provider == 'webnn') {
            session_options.freeDimensionOverrides = {
                'max_seq_len': 1,
            ...
            log('Create session for decode process...done');
        }
        ...
        if (this.provider == 'webnn') {
            ...
            for (let i = 0; i < this.num_layers; ++i) {
                ...
                this.feed[`past_key_values.${i}.decoder.value`] =
                    ort.Tensor.fromMLTensor(input_ml_tensor, ort_kv_desc);
                // output fetches
                this.prefill_fetches[`new_present_key_values.${i}.decoder.key`] =
                    ort.Tensor.fromMLTensor(prefill_key_ml_tensor, ort_kv_desc);
                this.prefill_fetches[`new_present_key_values.${i}.decoder.value`] =
                    ort.Tensor.fromMLTensor(prefill_value_ml_tensor, ort_kv_desc);
                this.decode_fetches[`new_present_key_values.${i}.decoder.key`] =
                    ort.Tensor.fromMLTensor(decode_key_ml_tensor, ort_kv_desc);
                this.decode_fetches[`new_present_key_values.${i}.decoder.value`] =
                    ort.Tensor.fromMLTensor(decode_value_ml_tensor, ort_kv_desc);
                this.decode_tmp_fetches[`new_present_key_values.${i}.decoder.key`] =
                    ort.Tensor.fromMLTensor(decode_tmp_key_ml_tensor, ort_kv_desc);
                this.decode_tmp_fetches[`new_present_key_values.${i}.decoder.value`] =
                    ort.Tensor.fromMLTensor(decode_tmp_value_ml_tensor, ort_kv_desc);
            }
            const token_id_desc = {
                dataType: 'int32',
                shape: [1, 1],
            ...
            this.prefill_fetches['token_id'] =
                ort.Tensor.fromMLTensor(this.prefill_token_id_tensor, ort_token_id_desc);

            this.decode_token_id_tensor = await this.ml_context.createTensor(token_id_desc);
            this.decode_fetches['token_id'] =
                ort.Tensor.fromMLTensor(this.decode_token_id_tensor, ort_token_id_desc);
            this.decode_tmp_fetches['token_id'] =
                ort.Tensor.fromMLTensor(this.decode_token_id_tensor, ort_token_id_desc);
    ...
    async generate(input_ids, cleanKV, callback) {
        ...
        while (this.eos.indexOf(last_token) == -1 && !this.stop) {
            ...
            if (this.provider == 'webnn') {
                ...
                console.time('decode session run');
                await this.sess_2.run(this.feed, outputs);
                console.timeEnd('decode session run');
            ...
            this.start_len += 1;
            seqlen += 1;
            console.timeEnd('decode processing');
        }
