...
bIndices[${x-2}] = u32(row);
  ...
  ${h?cn(y,b,K,Ae):dn(y,b,K,Ae)}
    ...
    fn mm_write(batch: i32, row : i32, colIn : i32, valueIn : ${_}) {
    ...
    }`},Fm=(r,e,n,t,o,i,s,a,u)=>{let l=e.format==="NHWC",c=l?r[0].dims[3]:r[0].dims[1],d=n[0],f=l?n[2]:n[3],m=l?n[1]:n[2],p=l?n[3]:n[1],h=l&&(c%4===0||c%3===0)&&p%4===0,y=l?p:f*m,b=l?f*m:p,g=[8,8,1],x=t<=8?[4,1,1]:[4,4,1],T=[Math.ceil(y/g[0]/x[0]),Math.ceil(b/g[1]/x[1]),Math.ceil(d/g[2]/x[2])];se("verbose",()=>`[conv2d_mm_webgpu] dispatch = ${T}`);let _=h?l&&c%4!==0?3:4:1,I=g[1]*x[1],O=g[0]*x[0],E=Math.max(g[0]*_,g[1]),z=t%I===0,N=o%O===0,D=i%E===0,le=h?[_,4,4]:[1,1,1],ee=[{type:6,data:t},{type:6,data:o},{type:6,data:i},{type:6,data:[e.pads[0],e.pads[1]]},{type:6,data:e.strides},{type:6,data:e.dilations}];at(e,ee),ee.push(...k(r[0].dims,r[1].dims));let ie=["rank","rank"];s&&(ee.push(...k(r[2].dims)),ie.push("rank")),ee.push(...k(n));let Ae=K=>{let fe=[{name:"dim_a_outer",type:"i32"},{name:"dim_b_outer",type:"i32"},{name:"dim_inner",type:"i32"},{name:"pad",type:"i32",length:2},{name:"stride",type:"i32",length:2},{name:"dilation",type:"i32",length:2}];st(e,fe);let ve=h?4:1,q=he(r[0].dataType),ue=`
      ...
      }`,te=P("x",r[0].dataType,r[0].dims.length,_===3?1:_),re=P("w",r[1].dataType,r[1].dims.length,ve),ne=[te,re],B=C("result",r[0].dataType,n.length,ve);if(s){let G=P("bias",r[2].dataType,r[2].dims.length,ve);ne.push(G),ue+=`
...
${B}_indices[${yt}] = 0;`}),Te+=`${B}_indices[${ne-2}] = 0u;
                     ${B}_indices[${ne-1}] = 0u;`,Te},ue=()=>{let te=`var a_data: ${O.type.value};`;for(let re=0;re<f;re++)te+=`
...
`}return te};return`
  ...
  }`},eh=(r,e,n,t,o,i,s,a)=>{let u=e.format==="NHWC",l=u?r[0].dims[3]:r[0].dims[1],c=n[0],d=u?n[2]:n[3],f=u?n[1]:n[2],m=u?n[3]:n[1],p=u&&l%4===0&&l%3&&m%4===0,h=u?m:d*f,y=u?d*f:m,b=[8,8,1],g=t<=8?[4,1,1]:[4,4,1],x=[Math.ceil(h/b[0]/g[0]),Math.ceil(y/b[1]/g[1]),Math.ceil(c/b[2]/g[2])];se("verbose",()=>`[conv_backprop_mm_webgpu] dispatch = ${x}`);let T=p?4:1,_=Math.max(b[0]*T,b[1]),I=p?4:1,O=[e.kernelShape[u?1:2],e.kernelShape[u?2:3]],E=[O[0]+(e.dilations[0]<=1?0:(O[0]-1)*(e.dilations[0]-1)),O[1]+(e.dilations[1]<=1?0:(O[1]-1)*(e.dilations[1]-1))],z=[E[0]-1-Math.floor((e.pads[0]+e.pads[2])/2),E[1]-1-Math.floor((e.pads[1]+e.pads[3])/2)],N=[{type:6,data:t},{type:6,data:o},{type:6,data:i},{type:6,data:e.strides},{type:6,data:e.dilations},{type:6,data:O},{type:6,data:z}];at(e,N),N.push(...k(r[0].dims,r[1].dims));let D=["rank","rank"];s&&(N.push(...k(r[2].dims)),D.push("rank")),N.push(...k(n));let le=ee=>{let ie=P("x",r[0].dataType,r[0].dims.length,I),Ae=P("w",r[1].dataType,r[1].dims.length,1),K=C("result",r[0].dataType,n.length,I),fe=[ie,Ae],ve="";if(s){let te=P("bias",r[2].dataType,r[2].dims.length,I);fe.push(te),ve+=`
          fn getBiasByOutputCoords(coords : vec4<i32>) -> ${te.type.value} {
            return bias[coords.${u?"w":"y"}${p?"/ 4":""}];
          }`}let q=[{name:"dim_a_outer",type:"i32"},{name:"dim_b_outer",type:"i32"},{name:"dim_inner",type:"i32"},{name:"strides",type:"i32",length:2},{name:"dilations",type:"i32",length:2},{name:"filter_dims",type:"i32",length:O.length},{name:"pads",type:"i32",length:z.length}];st(e,q);let ue=he(r[0].dataType,1);if(ue!=="f16"&&ue!=="f32")throw new Error(`elemType ${ue} is not supported.`);return`
        ${Uo("uniforms.result_strides")}
        ${ee.registerUniforms(q).declareVariables(...fe,K)};
        ${ve}
        ${gv(u,s,e,ie.type.value,T)}
        ${p?cn(g,b,ue,void 0,!u,_):dn(g,b,ue,void 0,!u,_,!1,void 0,a)}`};return{name:"Conv2DTransposeMatMul",shaderCache:{hint:`${e.cacheKey};${g};${b};${p}`,inputDependencies:D},getRunData:()=>({outputs:[{dims:n,dataType:r[0].dataType}],dispatchGroup:{x:x[0],y:x[1],z:x[2]},programUniforms:N}),getShaderSource:le}}});var bv,Ga,rh=S(()=>{"use strict";j();St();Y();Q();bv=(r,e,n,t,o,i=!1,s,a,u=!1)=>{let l=u?1:2,c=u?2:3,d=u?3:1,f=i?2:1,m=`
        ...
        ${D.mainStart([b,y,1])}
          ...
          for (var tile: u32 = 0; tile < num_tiles; tile += 1) {
            ...
            for (var i: u32 = 0; i < ${m}; i++) {
              ...
              let b_quantized_values = mat2x4<${ue}>(${Array.from({length:4},(re,ne)=>`${ue}(b_value_lower[${ne}]), ${ue}(b_value_upper[${ne}])`).join(", ")});
