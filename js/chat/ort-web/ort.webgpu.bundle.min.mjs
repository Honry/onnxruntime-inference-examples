  ...
  ${$.mainStart()}
    ...
    ${b.setByOffset("global_idx",w.getByIndices("aIndices"))}
       ...
       `;return{name:e,shaderCache:t,getShaderSource:g=>`
        ...
        }`,getRunData:()=>({outputs:[{dims:i,dataType:o}],dispatchGroup:{x:l},programUniforms:[{type:12,data:c}]})}},tt=(e,t,r,n)=>{let o=e.inputs.length===1?r:uo(e.inputs,r),i=o.axes;i.length===0&&!o.noopWithEmptyAxes&&(i=e.inputs[0].dims.map((w,b)=>b));let a=k.normalizeAxes(i,e.inputs[0].dims.length),d=a,l=e.inputs[0],c=om(d,e.inputs[0].dims.length);c.length>0&&(l=e.compute(Ee(e.inputs[0],c),{inputs:[0],outputs:[-1]})[0],d=em(d.length,l.dims.length));let[m,u]=tm(l.dims,d),h=m;o.keepDims&&(h=rm(m,a)),e.compute(im(t,{hint:o.cacheKey,inputDependencies:["type"]},[l],n,e.inputs[0].dataType,h,u),{inputs:[l]})},ns=(e,t)=>{tt(e,"ReduceMeanShared",t,"mean")},os=(e,t)=>{tt(e,"ReduceL1Shared",t,"l1")},is=(e,t)=>{tt(e,"ReduceL2Shared",t,"l2")},as=(e,t)=>{tt(e,"ReduceLogSumExpShared",t,"logSumExp")},ss=(e,t)=>{tt(e,"ReduceMaxShared",t,"max")},us=(e,t)=>{tt(e,"ReduceMinShared",t,"min")},ds=(e,t)=>{tt(e,"ReduceProdShared",t,"prod")},ls=(e,t)=>{tt(e,"ReduceSumShared",t,"sum")},cs=(e,t)=>{tt(e,"ReduceSumSquareShared",t,"sumSquare")},ps=(e,t)=>{tt(e,"ReduceLogSumShared",t,"logSum")}});var rt,am,Fr,uo,nt,sm,um,dm,lm,cm,pm,mm,fm,hm,gm,ot,fs,hs,gs,bs,ys,ws,_s,vs,$s,xs,Hr=R(()=>{"use strict";te();ae();Ie();se();ms();rt=e=>{if(!e||e.length===0||e.length>2)throw new Error("Reduce op requires 1 or 2 inputs.");if(e.length===2&&e[1].dims.length!==1)throw new Error("Invalid axes input dims.")},am=e=>["","",`var value = ${e.getByIndices("input_indices")};`,""],Fr=(e,t,r,n,o,i,a=!1,d=!1)=>{let l=[],c=r[0].dims,m=c.length,u=k.normalizeAxes(o,m),h=!d&&u.length===0;c.forEach((x,$)=>{h||u.indexOf($)>=0?a&&l.push(1):l.push(x)});let w=l.length,b=k.size(l);return{name:e,shaderCache:t,getShaderSource:x=>{let $=[],v=E("_A",r[0].dataType,m),S=M("output",i,w),T=n(v,S,u),I=T[2];for(let A=0,P=0;A<m;A++)h||u.indexOf(A)>=0?(a&&P++,I=`for(var j${A}: u32 = 0; j${A} < ${c[A]}; j${A}++) {
...
var best_index : i32 = 0;`,`if (${n.getByIndices("input_indices")} ${t.selectLastIndex>0?">=":">"} value) {
  ...
  }`}},Ps=e=>{Tm(e.inputs),e.compute(Cm(e.inputs))}});var Im,be,Os,Ds,Bs,Ms,Rs,Us,Vs,Ns,Ws,Am,Ls,Gs,Hs,Fs,Zt,qs,jr,js,Ks,Ys,Zs,Xs,Qs,Js,eu,tu,ru,nu,ou,iu,au,su,uu,du,lu,po,mo,cu,pu,mu,km,Em,fu,Kr=R(()=>{"use strict";te();ae();Ie();se();Im=(e,t,r,n,o,i,a)=>{let d=Math.ceil(t/4),l="";typeof o=="string"?l=`${o}(a)`:l=o("a");let c=E("inputData",r,[d],4),m=M("outputData",n,[d],4),u=[{name:"vec_size",type:"u32"}];return a&&u.push(...a),`
...
`});var Nm,Wm,Qt,zu,Lm,Jt,Gm,Qr,er=R(()=>{"use strict";te();ae();se();dt();Xt();Nm=(e,t)=>e?`
...
bIndices[${S-2}] = u32(row);
    ...
    `},Qr=(e,t,r,n,o=!1,i)=>{let a=e[0].dims,d=e[1].dims,l=a.slice(0,-2),c=d.slice(0,-2),m=n?n.slice(0,-2):r.slice(0,-2),u=k.size(m),h=a[a.length-2],w=a[a.length-1],b=d[d.length-1],g=w%4===0&&b%4===0,x=h<=8?[4,1,1]:[4,4,1],$=[8,8,1],v=[Math.ceil(b/$[0]/x[0]),Math.ceil(h/$[1]/x[1]),Math.ceil(u/$[2]/x[2])],S=g?4:1,T=[...l,h,w/S],I=T.length,A=[...c,w,b/S],P=A.length,D=[u,h,b/S],N=[{type:6,data:h},{type:6,data:b},{type:6,data:w}];We(t,N),N.push(...V(m,T,A));let G=["rank","rank"],q=e.length>2;q&&(N.push(...V(e[2].dims)),G.push("rank")),N.push(...V(D));let K=W=>{let J=m.length,ue=Gr("batchDims",e[0].dataType,J,1),ee=ge(e[0].dataType),oe=E("a",e[0].dataType,I,S),X=E("b",e[1].dataType,P,S),Q=M("result",e[0].dataType,D.length,S),ye=[oe,X];if(q){let H=o?S:1;ye.push(E("bias",e[2].dataType,e[2].dims.length,H))}let we=[{name:"dim_a_outer",type:"i32"},{name:"dim_b_outer",type:"i32"},{name:"dim_inner",type:"i32"}];Le(t,we);let ce=ge(Q.type.tensor),ne=Ne(t,Q.type.value,ce),U=Gm(S,q,ne,[ue,oe,X,Q],[l,c,m],o);return`
  ...
  }`};return{name:"GroupedConv-Vectorize",shaderCache:{hint:`${t.cacheKey};${i};${a};${h};${c[0]};${c[1]}`,inputDependencies:o?["rank","rank","type"]:["rank","rank"]},getRunData:()=>({outputs:[{dims:n?n(r):r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(d/64)},programUniforms:u}),getShaderSource:w}}});var ho,Km,Gu,go=R(()=>{"use strict";te();ae();er();se();dt();ho=(e,t,r,n,o=!1,i)=>{let a=e[0].dims,d=e[1].dims,l=a[a.length-2],c=d[d.length-1],m=a[a.length-1],u=$e(c),h=$e(m),w=$e(l),b=k.size(r)/u/w,g=e.length>2,x=n?n.slice(0,-2):r.slice(0,-2),v=[k.size(x),l,c],S=[{type:12,data:b},{type:12,data:l},{type:12,data:c},{type:12,data:m}];We(t,S),S.push(...V(x,a,d)),g&&S.push(...V(e[2].dims)),S.push(...V(v));let T=I=>{let A=Gr("batch_dims",e[0].dataType,x.length),P=E("a",e[0].dataType,a.length,h),D=E("b",e[1].dataType,d.length,u),N=M("output",e[0].dataType,v.length,u),G=ge(N.type.tensor),q=Ne(t,N.type.value,G),K=[P,D],W="";if(g){let we=o?u:1;K.push(E("bias",e[2].dataType,e[2].dims.length,we)),W=`${o?`value += bias[col / ${we}];`:`value += ${N.type.value}(bias[row + i]);`}`}let J=a.slice(0,-2),ue=d.slice(0,-2),ee=Bt(J,x),oe=Bt(ue,x),X=[{name:"output_size",type:"u32"},{name:"M",type:"u32"},{name:"N",type:"u32"},{name:"K",type:"u32"}];Le(t,X);let Q=(we,ce)=>{let ne=we.rank,U=we.name;if(ne===2)return`var ${U}_indices = ${we.type.indices}(0u, 0u);`;let H=A.rank,pe=`var ${U}_indices: ${we.type.indices};`;for(let Oe=ne-2-1,he=H-1;Oe>=0;Oe--,he--)pe+=`
